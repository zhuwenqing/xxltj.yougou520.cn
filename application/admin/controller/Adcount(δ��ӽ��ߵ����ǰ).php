...
class Adcount extends AdminBase
{
    ...
    public function index($cid = 4,$page = 1,$keyword = ''){
        ...
        if($cid == 0){
            $url_list = $this->adcount_model->alias('a')->field('a.*,think_adcategory.name')->where($map)->join('think_adcategory','a.cid = think_adcategory.id')->order('a.sort DESC')->paginate(13,false,['page'=>$page,'query'=>['cid'=>$cid]]);
        }else{
        ...
        // $redis = new Redis();
        // $redis_url_list = $redis->get('url_list');
        // $arr = [];
        // foreach($redis_url_list as $k=>$v){
            // $arr[$v['cid']][] = $v;
        // }
        // dump($arr);exit();
    ...
    function exportExcel($filename = '',$startRow=1,$excel2007=false){  
        ...
        $count_detail_list = Db::name('count_detail_'.TIME_PREFIX)->field('count_ip,copu_wechat,copy_time,origin_keyword,visit_time')->where($map)->order('visit_time desc')->select();
        
          
        $indexKey = array('count_ip','copu_wechat','copy_time','origin_keyword','visit_time'); 
        $list_data = array(array('count_ip'=>'用户ip','copu_wechat'=>'复制微信号','copy_time'=>'复制时间','origin_keyword'=>'复制来源关键词','visit_time'=>'访问时间')); 
