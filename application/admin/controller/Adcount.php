...
class Adcount extends AdminBase
{
    ...
    public function index($cid = 4,$page = 1,$keyword = '',$ad_order = 'sort'){
        ...
        if(!empty($keyword)){
            $map['count_url|remark']=['like','%'.$keyword.'%'];
            $cid = 0;
        } 
        if($cid == 0){
    ...
    public function save(){
        ...
        if($validate_result !== true){
            ...
            $result = $this->adcount_model->allowField(true)->insertGetId($data);
            if ($result) {
                $optye = '保存统计域名- '.$data['count_url'];
                add_logs($optye,1);
                $this->success('保存成功',url('admin/adcount/ad_code',array('id'=>$result)));
            } else {
                $this->error('保存失败');
            }
        }
    ...
    ////统计域名详情
    public function detail($id,$key_name = '',$keyword = '',$page = 1,$start_time = '',$end_time = '',$is_copy = '',$adcount_time=''){
        $url = Db::name('adcount')->field('count_url')->where('id',$id)->find();
        $this->assign('count_url',$url['count_url']);
        $search_list = [
            [
                'name'=>'IP',
                'value'=>'count_ip'
    ...
    public function wechat_count($id,$key_name = '',$keyword = '',$page = 1,$start_time = '',$end_time = '',$is_copy = ''){
        ...
        $wechatName = Db::name('count_detail_'.TIME_PREFIX)->field('count(*) as count,copu_wechat,id,cid')->where('cid',$id)->where($map)->group('copu_wechat')->select();
    ...
    //删除统计域名
    function delete($id){
        $count_detail = Db::name('count_detail_'.TIME_PREFIX)->where('cid',$id)->select();
        $url_name = Db::name('adcount')->field('count_url')->where('id',$id)->find();
        if(empty($count_detail)){
            $data_adcount = Db::name('adcount')->where('id',$id)->delete();     
            if($data_adcount){
                $optye = '删除统计域名- '.$url_name['count_url'];
                add_logs($optye,3);
                $this->success('删除成功');
            }else{
                $this->error('保存失败');
            ...
            Db::transaction(function(){
                $data_detail = Db::name('count_detail_'.TIME_PREFIX)->where('cid',$id)->delete();                
                $data = Db::name('adcount')->where('id',$id)->delete();
                if($data && $data_detail){
                    $optye = '删除统计域名- '.$url_name['count_url'];
                    add_logs($optye,3);
                    $this->success('删除成功');
    ...
    public function address_count($id,$area = 'province',$keyword='',$start_time = '',$end_time = ''){
    ...
    public function advdata_page($id,$start_time = '',$end_time = ''){
        ...
        $adv_data = $this->advdata_model->where('cid',$id)->where($map)->select();
    ...
    //投诉统计
    public function complaint($id,$start_time = '',$end_time = ''){
        $map['cid'] = ['eq',$id];
        $result = $this->complaint_model
                        ->alias('c')
                        ->field('count(*) as count,think_complaint_name.name')
                        ->join('think_complaint_name','c.comp_id = think_complaint_name.id')
                        ->where($map)
                        ->group('comp_id')
                        ->select();
        $this->assign('comp_total',$result);
