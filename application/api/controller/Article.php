...
//        dump($readd);
    ...
    public function mp_category(){
        $category = Db::name('artcategory')->where('status',1)->order('id asc')->select();
     ...
     *
     * */
    public function mp_detail($id=0){
        if(!empty($id)){
            // 根据id查询数据
            $re =  $this->wearticle_model->showOneArticle($id);
//            $re['0']['content'];
