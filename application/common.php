...
//添加日志
function add_logs($optye,$type = 0){
    // $data   一般包含
    if(array_key_exists('HTTP_X_FORWARDED_FOR',$_SERVER)){
        $reIP=$_SERVER['HTTP_X_FORWARDED_FOR'];
    }else{
        $reIP=$_SERVER['REMOTE_ADDR'];
    }
    $username = Session::get('admin_name');
    $operate = '登录者:'.$username.' 操作- '.$optye;        
    // 操作类型  1  新增  2 修改  3 删除  默认是0 其他操作
    $data = array(
        'ip' =>$reIP ,
        'type' => $type,
        'detail' => $operate ,
    ...
    $result = Db::name('logs')->insert($data);
    return $result;
}